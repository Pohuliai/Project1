...
$(document).ready(function () {
    if(JSON.parse(localStorage.getItem("cart")) == null){
        ...
        for (let key in cart) {
            ...
            $(`div[data-id=${key}]`).find(".product-price").find(".amountBlock").addClass("shown").find("input").val(cart[key])
            let count = $(".price-in-bag").text()
            let amount = parseInt(count) + cart[key]
            $(".price-in-bag").text(amount.toString())
            console.log(amount, typeof(amount))
            }
            addToCart($(`div[data-id=${key}]`))}
        }
    }
...
function addToCart(element) {
    ...
    //console.log(idItem)
    let amount = 1
    if(element.hasClass("product-item")){
        amount = $(element).find(".product-price").find(".amountBlock.shown").find("input").val()
        idItem = $(element).attr("data-id")
        name = element.find(".product-name-descr").find("h3").text()
        price = element.find(".product-price").find(".price").text()
        img = element.find(".product-image").find(".img-wrap").find("img").attr("src")
        //id = element.parent().find("input").attr("data-id")
    } 
